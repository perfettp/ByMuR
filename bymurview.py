            ...
            if (self._mapPath):
                ...
                if (ext != "png"):
                    msg = ("ERROR:\nYou can upload .png file "
                           "format only.")
                    bymurMessage(parent=self._parent,
                ...
                bymurMessage(parent=self._parent, kind="BYMUR_ERROR",
                             caption="Error!",
...
class BymurDBCreateDlg(wx.Dialog):
    def __init__(self, *args, **kwargs):
        ...
        self._localMapDefaults = {'lat_min': kwargs.pop('lat_min'),
                                  'lat_max': kwargs.pop('lat_max'),
                                  'lon_min': kwargs.pop('lon_min'),
                                  'lon_max': kwargs.pop('lon_max'),
...
class BymurEnsembleDlg(wx.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(BymurEnsembleDlg, self).__init__(style=self._style, *args, **kwargs)
            ...
            for i in range(len(self._ensBoxSizer.hazArray)):
                if self._ensBoxSizer.hazArray[i]['checkbox'].IsChecked():
                    ens_item = {
                        ...
                        'weight': float(self._ensBoxSizer.hazArray[i]['text'].GetValue())
                    }
                    ensLocalDetails['components'].append(ens_item)
            if len(ensLocalDetails['components']) < 2:
                bymurMessage(parent=self, kind="BYMUR_ERROR",
...
class BymurWxView(wx.Frame):
    ...
    def busy(self, state):
        ...
        if self._isbusy:
            ...
            # TODO: why the text is not rendered?
            self._busydlg = BymurBusyDlg(self._busymsg, parent=self)
            self.PushStatusText(self._busymsg)
            self.Refresh()
            self.Update()
