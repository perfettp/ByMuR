...
def dbConnection(*kargs):
  ...
  try:
    ...
    txt = "\nConnection Succeded!\nMySQL version : {0} ".format(db_version[0])
...
#        curdir = os.path.join(hazpath, hazards[h], models[h][m], dtime[k])
        ...
        curdir = os.path.join(hazpath, hazards[h], models[h][m], dtimefold[nmods-1][k])
...
# FINE JACOPO 18.06.2013    
        if not os.listdir(curdir):
          ...
          hcTmp = [[["0"]*npts]*len(perc)]*len(dtime[nmods-1])
        else:  
...
#          print 'file -->',filexml
          ...
          for p in perc:
            ...
            if os.path.isfile(filexml):
              ...
              tmplist = [' '.join('0.0 ' for i in range(niml))] * npts
      ...
      for m in range(len(models[h])):
        ...
        if cur.fetchone()[0]:
          sql = """ 
                ...
                WHERE id_haz={4}
                """
          cur.execute(sql.format(hazards[h], models[h][m], 
                                 imt[nmods-1].replace("_"," "), iml[nmods-1], nmods))
        else:
          sql = """ 
                ...
                ({0}, '{1}', '{2}', '{3}', '{4}', 1, 1, 0)
                """
          cur.execute(sql.format(nmods, hazards[h], models[h][m], 
                                 imt[nmods-1].replace("_"," "), iml[nmods-1]))
        ...
        for k in range(len(dtime[m])):
          for i in range(npts):        
            ...
            sql = """
                  ...
                  """
            cur.execute(sql.format(tbname, m+1, i+1, "Average", 
                                   dtime[m][k], mean[m][k][i], idc))
          ...
          for p in range(len(perc)):
            ...
            for i in range(npts):        
              ...
              sql = """
                    ...
                    dtime='{4}', curve='{5}' WHERE id={6}
                    """
              cur.execute(sql.format(tbname, m+1, i+1, "Perc"+pp, 
                                     dtime[m][k], hc[m][k][p][i], idc))
        ...
        for k in range(len(dtime[m])):
          for i in range(npts):        
            ...
            sql = """
                  ...
                  """
            cur.execute(sql.format(tbname, idc, m+1, i+1, "Average", 
                                   dtime[m][k], mean[m][k][i]))
          ...
          for p in range(len(perc)):
            ...
            for i in range(npts):        
              ...
              sql = """
                    ...
                    dtime, curve) VALUES ( {1}, {2}, {3}, '{4}', '{5}', '{6}' ) 
                    """
              cur.execute(sql.format(tbname, idc, m+1, i+1, "Perc"+pp, 
                                     dtime[m][k], hc[m][k][p][i]))
