...
# END JACOPO 10/06/13
  ...
  limits = [375.300, 508.500, 4449.200, 4569.800]        # limits of image map
  imgpath = os.path.join(srcdir, "data", "naples.png")   # image map path
  gridpath = os.path.join(srcdir, "data", "naples-grid.txt") # image map path
...
#    self.SetIcon(wx.Icon("icons/", wx.BITMAP_TYPE_ANY))
    ...
    load_bymurDBitem = wx.MenuItem(self.fileMenu, 103, '&Load remote ByMuR DB')
    #quit_item.SetBitmap(wx.Bitmap('icons/exit.png'))
    ...
    self.map_export_gis = wx.MenuItem(self.plotMenu, 121, '&Export Raster ASCII (GIS)')
...
# ROBERTO 13/06/2013
    #ensemble_item = wx.MenuItem(self.analysisMenu, 211, 'Create &Ensemble hazard')
    self.ensemble_item = wx.MenuItem(self.analysisMenu, 211, 'Create &Ensemble hazard')
...
# END JACOPO 4/6/13
    ...
    hbox_haz = wx.StaticBoxSizer(wx.StaticBox(self.pnl_lt, wx.ID_ANY, 
                                 'Hazard'), orient=wx.HORIZONTAL)

    self.vbox_haz_lt = wx.BoxSizer(orient=wx.VERTICAL)
    self.vbox_haz_rt = wx.BoxSizer(orient=wx.VERTICAL)

    self.hlbl = wx.StaticText(self.pnl_lt, wx.ID_ANY, "Select Hazard Model:")
    ...
    self.vbox_haz_lt.Add(wx.StaticText(self.pnl_lt, wx.ID_ANY, 
                         "Select Return Period (yr):"), 0, wx.TOP, 6)
    ...
    self.bpth = wx.Button(self.pnl_lt, wx.ID_ANY, 'Update Map', size=(-1,26))
    ...
    self.vbox_haz_lt.Add(wx.StaticText(self.pnl_lt, wx.ID_ANY, 
                         "Select Intesity Threshold (0-1):"), 0, wx.TOP, 6)
    ...
    self.bith = wx.Button(self.pnl_lt, wx.ID_ANY, 'Update Map', size=(-1,26))
  ...
  def exportAsciiGis(self, event):
    ...
    if (dlg.ShowModal() == wx.ID_OK):
      ...
      for i in range(self.npts):
        fp.write("%f %f %f\n" %(self.lon[i]*1000, self.lat[i]*1000, self.zmap[i]))
...
# END ROBERTO 27/06/2013
  ...
  def dropAllTabs(self, event):
    ...
    if (answer == wx.ID_YES):
      con, cur = db.dbConnection("localhost", "V1User", "V1Pwd", "DPC-V1-DB")
...
# JACOPO 7/6/13
    ...
    if (tmp == 0):
      ...
      wx.Yield()
      db.dbReadHC(self.haz_sel, self.tw, self.dtime, self.cur, self.hc, self.hc_perc)
      busydlg = None
# END JACOPO 7/6/13

    self.zmap = self.pn1.hazardMap(self.hc, self.lon, self.lat, self.id_area,  
                       self.nareas, self.npts, self.hazards, self.dtime, 
                       self.haz_sel, self.tw, self.th, self.intTh, self.perc, 
                       self.imgpath, self.limits, self.iml, self.imt)
# JACOPO 3/6/13
#    self.pn2.hazardCurve(self.hc, self.haz_sel, self.tw, self.hazards,
#                         self.dtime, self.pt_sel, self.perc, self.iml, 
#                         self.imt)
...
# JACOPO 10/6/13
    print '----------selTimeWindow-------------'
    self.th = scientLibs.prob_thr(self.RP, self.dtime[self.haz_sel][self.tw])
...
# END ROBERTO 26.09.2013
  ...
  def openCreateDB(self, event):
    ...
    vbox_geo = wx.StaticBoxSizer(wx.StaticBox(self.fr, wx.ID_ANY, 
                                 'Geographical grid data'), orient=wx.VERTICAL)
    ...
    vbox_geo.Add(wx.StaticText(self.fr, wx.ID_ANY, label=txt, size=(-1,-1)), 
                               0, wx.TOP, 5)
    ...
    self.ptbut = wx.Button(self.fr, wx.ID_ANY, "Select File", size=(-1,26))
    self.Bind(wx.EVT_BUTTON, self.selGridFile, self.ptbut)
    hbox_pt.Add(self.ptbut, 0, wx.LEFT, 5)

    self.label_ptbut = wx.StaticText(self.fr, wx.ID_ANY, label="", size=(-1,-1))
    ...
    vbox_map = wx.StaticBoxSizer(wx.StaticBox(self.fr, wx.ID_ANY, 
    ...
    hbox1.Add(wx.StaticText(self.fr, wx.ID_ANY, label="Latitude Min (m):", 
                            size=(140,-1), style=wx.ALIGN_LEFT|wx.TE_WORDWRAP),
                            0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 2)
    self.lat_min = wx.TextCtrl(self.fr, wx.ID_ANY, "4449200", size=(120,-1))
    ...
    hbox2.Add(wx.StaticText(self.fr, wx.ID_ANY, label="Latitude Max (m):", 
                            size=(140,-1), style=wx.ALIGN_LEFT|wx.TE_WORDWRAP),
                            0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 2)
    self.lat_max = wx.TextCtrl(self.fr, wx.ID_ANY, "4569800", size=(120,-1))
    ...
    hbox3.Add(wx.StaticText(self.fr, wx.ID_ANY, label="Longitude Min (m):", 
                            size=(140,-1), style=wx.ALIGN_LEFT|wx.TE_WORDWRAP),
                            0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 2)
    self.lon_min = wx.TextCtrl(self.fr, wx.ID_ANY, "375300", size=(120,-1))
    ...
    vbox_con = wx.StaticBoxSizer(wx.StaticBox(self.fr, wx.ID_ANY, 
                                 'Database Connection'), orient=wx.VERTICAL)
    ...
    self.server = wx.TextCtrl(self.fr, wx.ID_ANY, "localhost", size=(120,-1))
    ...
    hbox4.Add(wx.StaticText(self.fr, wx.ID_ANY, label="Database Name:", 
                            size=(120,-1), style=wx.ALIGN_LEFT),
                            0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
    self.dbname = wx.TextCtrl(self.fr, wx.ID_ANY, "DPC-V1-DB", size=(120,-1))
    ...
    self.vbox_haz = wx.StaticBoxSizer(wx.StaticBox(self.fr, wx.ID_ANY, 
                                      'Hazard'), orient=wx.VERTICAL)
    
    self.vbox_haz.Add(wx.StaticText(self.fr, wx.ID_ANY, 
                                    label="Select hazard files directory:", 
                                    size=(-1,-1), style=wx.ALIGN_LEFT|wx.TE_WORDWRAP), 
                                    0, wx.TOP, 5)
    ...
    self.perc_lbl = wx.StaticText(self.fr, wx.ID_ANY, label=txt_perc, size=(-1,-1))
    ...
    self.canc_button = wx.Button(self.fr, wx.ID_ANY, "Cancel", size=(-1,28))
    self.Bind(wx.EVT_BUTTON, self.closeInputFr, self.canc_button)
    hbox_bot.Add(self.canc_button, 0, wx.TOP|wx.ALIGN_CENTRE, 10)
    self.save_button = wx.Button(self.fr, wx.ID_ANY, "Create", size=(-1,28))
...
# JACOPO 18.06.2013    
          ...
          dtime_tmp = [str(tmp[i].replace("dt","")).zfill(3) for i in range(len(tmp))]
...
## END JACOPO 10/06/13    
    ...
    busydlg = wx.BusyInfo("Task has been processing.. please wait", parent=self)
    wx.Yield()

    # comment the following two lines if DB tables exist and are populated
    db.dbCreateTables(self.con, self.cur, self.perc, self.hazards, self.models)
...
#                   hpath, self.npts)
    db.dbHazTabPop(self.con, self.cur, self.perc, self.hazards, self.models, self.dtime, 
                   hpath, self.npts, self.dtimefold)
...
#    self.sb.SetStatusText("Bymur Database successfully created")
  ...
  def openLoadDB(self, event):
    ...
    hbox1.Add(wx.StaticText(self.conFr, wx.ID_ANY, label="Server Name:", 
                            size=(120,-1), style=wx.ALIGN_LEFT),
                            0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
    self.server = wx.TextCtrl(self.conFr, wx.ID_ANY, "localhost", size=(120,-1))
    hbox1.Add(self.server, 0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
...
#    hbox2.Add(self.user, 0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
    self.user = wx.TextCtrl(self.conFr, wx.ID_ANY, "V1User", size=(120,-1))
    hbox2.Add(self.user, 0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
...
#    hbox4.Add(self.dbname, 0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
    self.dbname = wx.TextCtrl(self.conFr, wx.ID_ANY, "DPC-V1-DB", size=(120,-1))
    hbox4.Add(self.dbname, 0, wx.ALIGN_BOTTOM|wx.LEFT|wx.TOP, 0)
...
## END JACOPO 10/06/13    
    ...
    self.iml = [[float(j) for j in tmp[i][7].split()] for i in range(self.nhaz)]
...
# ROBERTO 11/09/2013
    ...
    for i in range(self.nhaz):
      ...
      self.perc_flag.append(db.dbAssignFlagPerc(self.con, self.cur, haznametb))
...
# ROBERTO 19/09/2013
    if (gf.verifyInternetConn()):
      ...
      #savepath = os.path.join("/localhome/bymuruser/ByMuR/last/data/naples_gmaps.png")
      utm_zone = "33N"
...
# END ROBERTO 19/09/2013
    ...
    self.th = scientLibs.prob_thr(self.RP, self.dtime[self.haz_sel][self.tw])
    self.hc = db.dbReadHC(self.haz_sel, self.tw, self.dtime, self.cur, self.hc, self.hc_perc)
#    print self.npts
#    tbname = "hazard" + str(1)
...
# ROBERTO 28/06/2013
  def openEnsembleFr(self, event):
    ...
    for i in range(self.nhaz):
      ...
      self.hazname[i] = wx.CheckBox(self.fr, wx.ID_ANY, label=self.model[i], size=(-1,-1))
...
# END ROBERTO 28/06/2013
  ...
  # update from db
    for i in range(len(self.selected)):
      ...
      if (tmp == 0):
        busydlg = wx.BusyInfo("...Reading hazard from DB")
        wx.Yield()
        db.dbReadHC(self.selected[i],self.twsel[i],self.dtime,self.cur,self.hc,self.hc_perc)
        busydlg = None
  ...
  # do ensemble
    ...
    tmp = scientLibs.ensemble(self.hc, self.hc_perc, self.tw, self.hazards,
                                 self.dtime,self.selected,self.weights,self.twsel,
                                 self.percsel,self.perc_flag)
  ...
  # update DB, table hazard_phenomena
    ...
    tmpmodel = tmpmodel + str(self.dtime[self.selected[0]][self.twsel[0]]) + ";" 
...
#      dt = str(dtimetmp[k]).zfill(3)
      ...
      for i in range(npts):        
        ...
        self.cur.execute(sql.format(tbname, idc, tmpid, i+1, "Average", 
                                   dt, hccomb[0][0][i]))
      for p in range(len(self.percsel)):
        ...
        for i in range(npts):        
          ...
          sql = """
                ...
                dtime, curve) VALUES ( {1}, {2}, {3}, '{4}', '{5}', '{6}' ) 
                """
#          tmp = ' '.join(map(str,hccomb[0,self.percsel[p],i][:]))     # or just p, as in dbFunctions.py???
          #print 'tmp--> ',tmp
          self.cur.execute(sql.format(tbname, idc, tmpid, i+1, "Perc"+pp, 
                                     dt, hccomb[0][self.percsel[p]][i])) 
    print 'DB populated!!'


# ROBERTO 11/09/2013 -- update true/false flag for percentiles in new hazard tab
