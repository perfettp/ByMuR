...
class BymurFrame(wx.Frame):
    ...
    def exportAsciiGis(self, event):
        ...
        if (dlg.ShowModal() == wx.ID_OK):
            ...
            for i in range(self.npts):
                fp.write("%f %f %f\n" % (self.lon[i] * 1000, self.lat[i] * 1000,
                    self.zmap[i]))
    ...
    #                             self.imt)
                    self.pn2.hazardCurve(self.hc, self.haz_sel, self.tw,
                                    self.hazards, self.dtime, self.pt_sel,
                                    self.perc, self.iml, self.imt, self.th,
                                    self.hc_perc, self.intTh)
    # FINE JACOPO 3/5/13
                else:
                    return
    ...
    # END JACOPO 7/6/13
        ...
        self.zmap = self.pn1.hazardMap(self.hc, self.lon, self.lat,
                        self.id_area, self.nareas, self.npts, self.hazards,
                        self.dtime, self.haz_sel, self.tw, self.th, self.intTh,
                        self.perc, self.imgpath, self.limits, self.iml,
                        self.imt)
    # JACOPO 3/6/13
    #    self.pn2.hazardCurve(self.hc, self.haz_sel, self.tw, self.hazards,
    #                         self.dtime, self.pt_sel, self.perc, self.iml,
    ...
    # END JACOPO 10/6/13
        ...
        self.zmap = self.pn1.hazardMap(self.hc, self.lon, self.lat,
                            self.id_area, self.nareas, self.npts, self.hazards,
                            self.dtime, self.haz_sel, self.tw, self.th,
                            self.intTh, self.perc, self.imgpath, self.limits,
                            self.iml, self.imt)
    ...
    #                   hpath, self.npts)
        db.dbHazTabPop(self.con, self.cur, self.perc, self.hazards, self.models,
                         self.dtime, hpath, self.npts, self.dtimefold)
    ...
    #      dt = str(dtimetmp[k]).zfill(3)
            ...
            for p in range(len(self.percsel)):
                ...
                for i in range(npts):
                    ...
                    #print 'tmp--> ',tmp
                    self.cur.execute(sql.format(tbname, idc, tmpid, i + 1,
                              "Perc" + pp, dt, hccomb[0][self.percsel[p]][i]))
        print 'DB populated!!'
