...
def utm2lola(x, y, z):
    ...
    q4 = (61 + 90 * t1 + 298 * c1 + 45 * pow(t1, 2) -
          3 * pow(c1, 2) - 252 * e2) * pow(d, 6) / 720
    q5 = d
    q6 = (1 + 2 * t1 + c1) * pow(d, 3) / 6
    q7 = (5 - 2 * c1 + 28 * t1 - 3 * pow(c1, 2) +
          8 * e2 + 24 * pow(t1, 2)) * pow(d, 5) / 120
...
def calcNxNy(lat_min, lat_max, lon_min, lon_max):
    ...
    while (nx > threshold or ny > threshold):
        ...
        # if zoom < 0:
        # zoom = 0
        xmin, xmax, ymin, ymax, nx, ny, dlon, dlat = calcNxNy(
